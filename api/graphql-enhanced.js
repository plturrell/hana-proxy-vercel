...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (query.includes('marketIntelligence')) {
      ...
      // Get data from both agents in parallel
      const [marketData, newsAnalysis, technicalAnalysis] = await Promise.all([
...
function generateBasicOpportunities(marketData, newsAnalysis, technicalAnalysis) {
  ...
  if (technicalAnalysis?.prediction === 'bullish' && newsAnalysis?.sentimentTrend === 'improving') {
    opportunities.push({
      type: 'momentum',
      ...
      risk_level: 'medium',
      timeframe: '1-2 weeks',
      entry_criteria: `Enter above $${technicalAnalysis?.indicators?.sma20}`,
      exit_criteria: `Target: $${technicalAnalysis?.resistance}, Stop: $${technicalAnalysis?.support}`,
      catalyst: 'Positive news sentiment with technical confirmation'
    });
  }
  ...
  if (technicalAnalysis?.indicators?.rsi < 30) {
    opportunities.push({
      ...
      direction: 'long',
      confidence: 0.6,
      potential_return: '3-7%',
      risk_level: 'medium',
      timeframe: '3-5 days',
      entry_criteria: 'RSI below 30 with positive divergence',
      exit_criteria: 'RSI above 50 or price at 20-day SMA',
      catalyst: 'Oversold conditions'
    });
  }
